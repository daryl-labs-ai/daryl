...
from ..types import MEF, ResolutionDecision, ResolutionNode
...
def make_resolution(
    *,
    ...
    ``producer`` is mandatory and should be a human/witnessed identity. ``confidence``
    is confidence in the *governance act* (the human decided), not the truth of the
    claim. MEF is complete-or-refuse. Pure — writes nothing.
    """
    mef = MEF(
        ...
        contested=contested,
        producer=producer,  # human/witnessed; MEF._non_empty enforces non-empty
    )
