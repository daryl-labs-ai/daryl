...
narrative.** Every meaningful line is backed by a DSM receipt. No LLM, no summarization,
no inference over the acts; if a fact has no receipt, it does not appear. This is a
projection over the certified chain — the mirror of "standing is derived, never stored".

Read-only, RR-only: assembles the two existing read paths (``ConsultationQuery`` for the
Proposal facet, ``StandingQuery`` for the Resolution facts + derived standing) over a
shared navigator. Imports only ``dsm.rr.*`` and receives ``storage`` — no writer.
"""
...
from .standing_read import ResolutionFact, StandingQuery
...
class ProposalFact:
    ...
    confidence: float
    answer: str
    receipt: str       # the Proposal Entry's DSM hash
    subject_id: str
...
class ExplainQuery:
    """Assembles the certified chain for a claim via RR (read-only). Reuses
    ``ConsultationQuery`` + ``StandingQuery`` over a shared navigator; the standing is
    ...
    def explain(self, claim_id: str) -> Explanation:
        ...
        # Observation is not a Proposal). First by RR order if several; None if absent —
