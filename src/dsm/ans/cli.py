...
# Package dsm is used via normal install
...
def cmd_recommend_task(args):
    ...
    if recommendations:
        for i, rec in enumerate(recommendations, 1):
            ...
            print(f"{priority_marker} {i}. {rec.skill_id} - {rec.reason}")
...
def cmd_risks(args):
    ...
    weak_skills = [rec for rec in report.recommendations if "avoid" in rec.reason.lower()]
    ...
    if weak_skills:
        ...
        for i, rec in enumerate(weak_skills, 1):
            print(f"{i}. {rec.skill_id} - {rec.reason}")
