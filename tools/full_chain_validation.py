...
# Add dsm root to path so "skills" resolves to memory/dsm/skills
...
def step_8_cli_checks():
    ...
    for cmd, args in cli_commands:
        cmd_line = [sys.executable, "-m", "dsm.skills.cli", cmd] + args
        print(f"\n  Testing: python -m dsm.skills.cli {cmd} {' '.join(args)}")
...
def step_9_verify_kernel_unchanged():
    ...
    frozen_files = [
        "dsm/core/storage.py",
        "dsm/core/models.py",
        "dsm/core/shard_segments.py",
        "dsm/core/security.py",
        "dsm/core/tracing.py",
        "dsm/session/session_graph.py",
        "dsm/session/session_limits_manager.py"
    ]
