...
def evaluate_leaf(
    ...
    # R12bis partitions the labels by merge strategy: a higher layer that CHANGED the value scores
    # `commutative_fold` under union/intersect and `layer_override` under replace/append. The strength
    # ordering therefore only ever discriminates `derogated_override` from the rest. (Journal E19.)
    changing_label = "commutative_fold" if merge in ("union", "intersect") else "layer_override"
    ...
    for rank in changed:
        ...
        if floor == "forbidden":
            ...
            continue
        if floor == "open":
            strength = max(strength, RULE_STRENGTH[changing_label])
            continue
