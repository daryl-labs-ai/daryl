...
- Highest migration cost of the three options. Updated estimate with SessionIndex in scope and Phase 7a.5 inserted : **~13–17 engineer-days** across 7 PRs (was ~8–12 / 6 PRs in v1 ; ~12–16 j after SessionIndex folded into Phase 7 ; +1 j for the Phase 7a.5 100 k-fixture benchmark re-run, no new prototype code).
...
- **Phase 7 (PR7, ~3–4 j) — SessionIndex deprecation.** Added in v2 after `SessionIndex` was classified `duplicative` (see `docs/architecture/ADR_0001_SESSIONINDEX_CLASSIFICATION.md`). Runs in three sub-steps (7a → 7a.5 → 7b), each with its own blocking exit criterion. 7b is gated on the combined PASS of 7a and 7a.5.
  ...
  - **7a.5 (blocking precondition for 7b, ~1 j of benchmark work, 0 new code).** Scalability validation on a production-representative dataset, inserted after Phase 7a's PASS verdict on 10 000 entries. **Goal :** validate that the operational cost of making RR the mandatory rebuild path for former SessionIndex consumers remains tolerable on a dataset size closer to production reality. **Scope :** re-run the existing Phase 7a harness (`benchmarks/bench_phase_7a_action_index.py`) on a **100 000-entry fixture** (10× the Phase 7a baseline), using the same two-dataset design (low cardinality / high cardinality). Prototype code under `src/dsm/rr/` stays **unchanged** from commit `58d7789` — no new prototype work is allowed in 7a.5 without reopening implementation explicitly. **What changes from Phase 7a :** fixture size 10 000 → 100 000 entries per dataset ; sessions ~500 → ~5 000 (maintain ~20 entries/session) ; everything else identical (same seeds, same action_name distributions, same 4 query variants, same median-of-5-runs build protocol). **Exit criterion (blocking for Phase 7b), three gates, all must hold on both datasets :**
    ...
    - **(ii) Query latency gate** — each of the 4 query variants `≤ 1.5× SessionIndex` equivalent. Same gate as Phase 7a.
    - **(iii) Operational absolute cost gate (new to 7a.5)** — `RR_with_action_build absolute ≤ 10× SessionIndex_build`. Rationale : the fixed overhead of RR's four cross-cutting indexes amortises partially with dataset size ; a 10× bound at 100 k is loose but real. A ratio `> 10×` means RR scales worse than SessionIndex on production-representative volumes, which invalidates the migration principle and forces RR optimisation before Phase 7b.
    **Consequences if 7a.5 FAILS :** Phase 7b is blocked ; RR must be optimised (options to evaluate, not to decide here : incremental rebuild, lazy index loading, shard-level index partitioning) ; **Phase 7a remains PASS** (architectural feasibility is unaffected) ; **SessionIndex classification remains `duplicative`** (a 6.44×-at-10k-that-drifts-past-10×-at-100k is an optimisation requirement on RR's pre-existing indexes, independent of the `action_name` extension — not grounds for reclassification). **Consequences if 7a.5 PASSES :** Phase 7b is unblocked ; transition `Proposed → Accepted` for ADR 0001 becomes defensible pending final review. **Non-goals of 7a.5 :** does not re-run the 10 k benchmark (results already published) ; does not modify the prototype ; does not rebind any SessionIndex consumer (that is 7b) ; does not modify kernel. **Exit artefact :** `docs/architecture/ADR_0001_PHASE_7A_5_VERDICT.md` with PASS/FAIL verdict per gate per dataset, JSON + markdown benchmark outputs under `benchmarks/results/phase_7a_5_action_index_<YYYYMMDD>.{json,md}`.
  - **7b.** Rebranch live SessionIndex consumers onto RR. Rewire `DarylAgent.index_sessions` / `find_session` / `query_actions` (`src/dsm/agent.py:620,625,630`) to call `RRIndexBuilder` / `RRNavigator` / `RRQueryEngine` respectively; preserve the public signatures. Rewire CLI `dsm session-index` / `session-find` / `session-query` / `session-list` (`src/dsm/cli.py:570–633`) — **CLI command names stay identical**, only the backend module changes. Rewire MCP `dsm_search` (`src/dsm/integrations/goose/server.py:378–402`) to call `agent.search_memory(action_name=...)` via the CL → RR path. Add `warnings.warn(DeprecationWarning)` in `SessionIndex.__init__` at `src/dsm/session/session_index.py:34`. Do NOT delete the module yet — that is scheduled for ADR 0001 + 6 months. Retire `tests/test_session_index.py` (242 L) once no production consumer remains. **Precondition :** Phase 7a.5 verdict is PASS. **Exit criterion :** `grep -rn "from .session.session_index import SessionIndex\|from dsm.session.session_index" src/dsm/ | grep -v 'session_index.py'` returns 0; `pytest` full suite green; CLI commands yield identical output on a fixture before and after the backend swap; all eight live consumers listed in the classification report are rebranched.

**Total : 7 phases (7a / 7a.5 / 7b counted as sub-phases of Phase 7), ~13–17 engineer-days** (was ~12–16 j in v2 before Phase 7a.5 was inserted ; 7a.5 adds ~1 j of benchmark work and produces no new prototype code).
...
### Q2 — Phase 7a.5 scalability verdict (blocking precondition for `Accepted`)
