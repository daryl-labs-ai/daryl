...
def verify_record(record: TraceRecord, strict: bool = False) -> tuple[bool, Optional[str]]:
    ...
    """
    # Recomputer le step_hash
    try:
        raw_data = json.loads(record.raw_line)
        computed_hash = compute_step_hash(raw_data)

        if strict and computed_hash != record.step_hash:
            return False, f"step_hash mismatch: computed={computed_hash[:16]}, stored={record.step_hash[:16]}"

        if not strict and computed_hash != record.step_hash:
            return True, f"step_hash mismatch: {record.step_hash[:16]} != {computed_hash[:16]}"
...
def replay_session(
    ...
    return ReplayReport(
        ...
        verified_records=len(records) - corrupt_count - broken_chain_count,
