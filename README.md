...
<img src="https://img.shields.io/badge/tests-260%20passing-brightgreen">
...
The kernel (`src/dsm/core/`) is **frozen since March 2026** — battle-tested, no modifications. Everything above it (P3–P9: SDK facade, pre-commitment, shard sealing, cross-agent receipts, session index, policy adapters, Ed25519 signing, artifact store) uses the public API without touching the internals.
...
```bash
pip install -e .[dev]
python -m pytest tests/ -v   # 260 tests, 0 failures
```
...
tests/          # 260 tests — core, session, rr, ans, P3-P9, security, integration
