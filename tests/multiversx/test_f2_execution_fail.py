...
ANDROMEDA_CONTAINING_NONCE = 1000000
...
def _receipt(regime: str) -> AnchorSubmissionReceipt:
...
class TestF2bAndromedaExecutionFail:
    ...
    def _run(self) -> list[Any]:
        transport = _make_transport(
            tx_sequence=[_load("andromeda/tx_fail.json")],
            block_by_nonce={
                ANDROMEDA_CONTAINING_NONCE: _load("andromeda/block_containing_tx.json"),
            },
        )
        http = httpx.Client(base_url="http://stub.invalid", transport=transport)
        watcher = MinimalPollingWatcher(http=http, regime="andromeda")
        return _drive_to_entries(watcher, _receipt("andromeda"), "andromeda")
